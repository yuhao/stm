...
void
run(uintptr_t id, int n_threads)
{
...
void*
//run_wrapper(void* i)
run_wrapper(void *t_tm)
{
	thread_tm_t tm = *(thread_tm_t *)t_tm;
    run((uintptr_t)tm.i, tm.n_t);
    TM_THREAD_SHUTDOWN();
    return NULL;
}
}
...
int main(int argc, char** argv) {
    ...
    for (uint32_t j = 1; j < CFG.threads; j++)
	{
		...
		t_tm.i = args[j];
		t_tm.n_t = CFG.threads;
        pthread_create(&tid[j], &attr, &run_wrapper, (void *)&t_tm);
	}
